...
with open(sys.argv[2],"w") as output_file:
    output_file.write("time-point\t" + CLIENT_MSG_CNT + "\t" + SERVER_MSG_CNT + "\tclient-bytes\tserver-bytes\tavg-client-bytes\tavg-server-bytes\n")
    for time_point in sorted(list(intervals.keys())):
        entry = intervals[time_point]
        avg_client_bytes = "0" if entry[CLIENT_MSG_CNT] == 0 else format(entry[CLIENT_BYTES_SUM] / entry[CLIENT_MSG_CNT],'.2f')
        avg_server_bytes = "0" if entry[SERVER_MSG_CNT] == 0 else format(entry[SERVER_BYTES_SUM] / entry[SERVER_MSG_CNT],'.2f')
        output_file.write(time_point + "\t" + str(entry[CLIENT_MSG_CNT]) + "\t" + str(entry[SERVER_MSG_CNT]) +
                          "\t" + str(entry[CLIENT_BYTES_SUM]) + "\t" + str(entry[SERVER_BYTES_SUM]) + "\t" + avg_client_bytes + "\t" + avg_server_bytes + "\n")